...
% 图表美化（方案 B）: 统一 caption 样式、使用 tabularx 和 threeparttable
...
\For{$i = 1$ to $h$}
    ...
    \State $\mathbf{attn}_i \leftarrow \text{softmax}(\bar{\mathbf{scores}}_i)$
...
项 & 说明 \\
\midrule
偶数维 ($2i$) & $PE_{(pos, 2i)} = \sin\left(\dfrac{pos}{10000^{2i/d_{model}}}\right)$：在偶数索引上使用正弦函数，随位置变化周期不同，用于编码绝对位置。 \\
奇数维 ($2i+1$) & $PE_{(pos, 2i+1)} = \cos\left(\dfrac{pos}{10000^{2i/d_{model}}}\right)$：在奇数索引上使用余弦函数，两者组合为每个位置生成唯一的向量。 \\
...
\begin{tablenotes}
\footnotesize
\item 注：$d_{ff}$ 通常设为 $4\times d_{model}$。
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\label{tab:ablation}
\begin{threeparttable}
\begin{tabularx}{0.9\textwidth}{@{}l*{4}{>{\centering\arraybackslash}X}@{}}
\toprule
...
项目代码已开源于 GitHub（https://github.com/your-username/transformer-implementation）。该仓库包含模型实现、训练与消融实验脚本、数据处理和工具函数、示例数据与结果输出，组织清晰便于复现与扩展：src/ 存放核心代码，scripts/ 为运行脚本，data/ 与 results/ 存放数据与实验结果，README.md 提供快速上手指南和运行示例。
